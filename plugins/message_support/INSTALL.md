...
Edit `message_support.php`: `ms_company_code`, `ms_company_name`, `ms_ticket_prefix`, `ms_hub_url`, `ms_api_token` (must match a row in the hub `tbl_support_company` table).
...
5. Register each new WD in `tbl_support_company` (`code`, `name`, `token`) and put the same token in that WD’s `message_support.php`.
